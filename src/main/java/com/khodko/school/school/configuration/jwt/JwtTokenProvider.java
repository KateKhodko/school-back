package com.khodko.school.school.configuration.jwt;
...
public class JwtTokenProvider {
	...
	private List<String> getListOfRolesName(List<Role> roles) {
	...
	public UserDetailsService userDetailsServiceBean() {
		return email -> {
			var expectedUser = Optional.ofNullable(userRepository.findByEmail(email))
					.orElseThrow(() -> new UsernameNotFoundException("Педик не найден"));

			return JwtUser.builder()
					.user(expectedUser)
					.enable(true)
					.authorities(List.of(new SimpleGrantedAuthority(expectedUser.getRole().name())))
					.build();
		};
	}
}
